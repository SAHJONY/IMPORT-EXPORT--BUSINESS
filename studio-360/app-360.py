...
# Free local engines (zero cost, zero license risk). Imported lazily-safe:
# if numpy/ffmpeg are missing the free modes report a clear error.
try:
    _fm_spec = importlib.util.spec_from_file_location(
        "free_music", os.path.join(BASE_DIR, "free_music.py"))
    ...
    _fm_mod = None
    FREE_MUSIC_OK = False
AUDIO_RE = re.compile(r"^([a-f0-9-]{8,64}|music-[a-f0-9-]{8,64})\.(mp3|wav)$")
...
VIDEO_RE = re.compile(r"^([a-f0-9-]{8,64}|video-[a-f0-9-]{8,64}|final-[a-f0-9-]{8,64})\.(mp4|mov)$")
...
def api_music_create():
    ...
    model = data.get("model") or "free"
    style = (data.get("style") or "regueton").strip()
    try:
    ...
    if model == "free":
        # ---- FREE local engine: zero cost, no key needed ----
        if not FREE_MUSIC_OK or _fm_mod is None:
            return jsonify({"ok": False, "error":
                            "El motor de música gratis no está disponible en "
                            "este servidor (falta numpy)."}), 500
        if style not in _fm_mod.STYLES:
            style = "regueton"
        label = _fm_mod.STYLES[style][0]
        job = _job_new("music", "🆓 " + (prompt[:40] or label),
                       {"free": True, "style": style, "secs": secs,
                        "prompt": prompt, "instrumental": instrumental},
                       cost_est=0)
        t = threading.Thread(target=_free_music_worker,
                             args=(job["id"], style, secs), daemon=True)
        t.start()
        return jsonify({"ok": True, "job_id": job["id"], "cost_est": 0})
    # ---- paid path (ElevenLabs) ----
    keys = _get_keys()
    if not keys["elevenlabs"]:
        return jsonify({"ok": False, "error":
                        "Falta la llave de ElevenLabs. Ponla en la pestaña ⚙️ Ajustes, "
...
def _free_music_worker(jid, style, secs):
    """Local free music generation: numpy synthesis -> MP3. Zero cost."""
    _job_update(jid, status="working",
                progress="Componiendo gratis en el estudio… 🎺 (tarda ~1 min)")
    tmp_wav = os.path.join(OUTPUT_DIR, f"tmp-{jid}.wav")
    fname = f"music-{jid}.mp3"
    dest = os.path.join(OUTPUT_DIR, fname)
    try:
        _fm_mod.generate(style, secs, tmp_wav)
        if (_job_get(jid) or {}).get("status") == "cancelled":
            return
        _fm_mod.to_mp3(tmp_wav, dest)
    except Exception as exc:  # noqa: BLE001
        _job_fail(jid, f"No se pudo componer: {exc}")
        return
    finally:
        try:
            os.remove(tmp_wav)
...
def api_video_create():
    ...
    model_key = data.get("model") or "free"
    image_url = (data.get("image_url") or "").strip()
    if model_key == "free":
        # ---- FREE local mode: still photo -> video with movement ----
        image_file = (data.get("image_file") or "").strip()
        if not image_file or not _resolve_media(image_file):
            return jsonify({"ok": False, "error":
                            "Sube la imagen primero. 🖼️"}), 400
        try:
            secs = int(data.get("secs") or 8)
        except (TypeError, ValueError):
            secs = 8
        secs = max(2, min(60, secs))
        job = _job_new("video", "🆓 " + (prompt[:50] or "Foto con movimiento"),
                       {"free": True, "image_file": image_file,
                        "secs": secs, "prompt": prompt},
                       cost_est=0)
        t = threading.Thread(target=_free_video_worker, args=(job["id"],),
                             daemon=True)
        t.start()
        return jsonify({"ok": True, "job_id": job["id"], "cost_est": 0})
    # ---- paid path (fal.ai) ----
    ...
    if not keys["fal"]:
        return jsonify({"ok": False, "error":
                        "Falta la llave de fal.ai. Ponla en la pestaña ⚙️ Ajustes, "
                        "o usa el modo 🆓 Gratis."}), 400
...
def _free_video_worker(jid):
    """Local free video: still photo -> vertical video with slow zoom
    (Ken Burns). Zero cost, pure FFmpeg."""
    job = _job_get(jid)
    if not job:
        return
    p = job["params"]
    src = _resolve_media(p.get("image_file") or "")
    if not src:
        _job_fail(jid, "No encontré la imagen subida. Súbela de nuevo. 🖼️")
        return
    secs = p.get("secs") or 8
    fname = f"video-{jid}.mp4"
    dest = os.path.join(OUTPUT_DIR, fname)
    _job_update(jid, status="working",
                progress="Dándole movimiento a tu foto… 🎥 (gratis)")
    frames = int(secs * 30)
    vf = ("scale=2160:3840:force_original_aspect_ratio=increase,"
          "crop=2160:3840,"
          "zoompan=z='min(zoom+0.0012,1.25)':d=" + str(frames) +
          ":x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1080x1920:fps=30")
    cmd = ["ffmpeg", "-y", "-v", "error", "-loop", "1", "-i", src,
           "-vf", vf, "-t", str(secs), "-c:v", "libx264",
           "-pix_fmt", "yuv420p", "-movflags", "+faststart", dest]
    try:
        subprocess.run(cmd, check=True, timeout=900)
    except Exception as exc:  # noqa: BLE001
        _job_fail(jid, f"No se pudo crear el video: {exc}")
        return
    if (_job_get(jid) or {}).get("status") == "cancelled":
        try:
            os.remove(dest)
