...
function checkRequiredFiles(){const required=['index.html','src/main.tsx','src/App.tsx','src/workflow.css','app/globals.css','public/landing.html','public/start.html','public/cuba-private-sector.html','public/cuba-es.html','public/cuba-us-desk-card.html','public/global-language.js','vercel.json','unified_api.py'];for(const f of required)exists(f)?pass(`Required file present: ${f}`):fail(`Required file missing: ${f}`)}
...
function checkVercelRoutes(){
  ...
  const exact=(src)=>routes.findIndex((r)=>r.src===src);const cubaPublic=exact('/cuba-private-sector');const cubaApi=routes.findIndex((r)=>r.src==='/cuba-private-sector/(health|leads.*)');const broadCuba=routes.findIndex((r)=>r.src==='/cuba(.*)');
  ...
  const criticalStatic={'/':'/landing.html','/business':'/landing.html','/start':'/start.html','/cuba-private-sector':'/cuba-es.html','/us-desk-card':'/cuba-us-desk-card.html'};for(const [src,dest] of Object.entries(criticalStatic)){const idx=routes.findIndex((r)=>r.src===src&&r.dest===dest);if(idx<0)fail(`Critical route missing or wrong: ${src} -> ${dest}`);else{const disk=`public${dest}`;exists(disk)?pass(`Critical static route verified: ${src} -> ${dest}`):fail(`Route ${src} points to missing static file ${disk}`)}}
  const ownerStatic={
