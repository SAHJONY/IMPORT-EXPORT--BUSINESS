...
function checkRequiredFiles(){const required=['index.html','src/main.tsx','src/App.tsx','src/workflow.css','app/globals.css','public/start.html','public/cuba-private-sector.html','public/cuba-es.html','public/cuba-us-desk-card.html','public/cuba-individual-consumers.html','public/global-language.js','vercel.json','unified_api.py'];for(const f of required)exists(f)?pass(`Required file present: ${f}`):fail(`Required file missing: ${f}`)}
