...
app = FastAPI(title="SAHJONY Voice Agent", version="1.0.0", docs_url=None, redoc_url=None)
...
def _task(context: str | None = None) -> str:
    extra = (context or "").strip()[:4000]
    return (
        "You are SAHJONY Global Trade's AI business phone agent. Speak naturally in the caller's language, "
        "defaulting to English and switching fluently to Spanish when appropriate. Disclose that you are an AI assistant. "
        "For buyers collect company, product, quantity/volume, destination, timing, Incoterm preference, payment capability, "
        "documentation readiness, and decision authority. For suppliers collect company, product/specification, capacity, MOQ, "
        "certifications, indicative FOB/CIF pricing, lead time, warranty when relevant, payment terms, and availability. "
        "Never claim QUALIFIED, APPROVED, CONTRACTED, PROVIDER_READY, sanctions-cleared, or credit-approved solely from the call. "
        "Never bind SAHJONY to price, purchase, sale, financing, payment, exclusivity, contract, KYC approval, sanctions approval, "
        "or other legal/commercial commitment. Escalate those matters to an authorized human. Respect do-not-call requests immediately. "
        "Do not record the call. End with a concise confirmed next step. "
...
async def _store(row: dict[str, Any]) -> None:
    try:
        ...
        # Telephony webhooks must remain acknowledgeable even during persistence degradation.
        pass
...
async def outbound(payload: OutboundCall, authorization: str | None = Header(None, alias="Authorization")):
    ...
    await _store({
        ...
        "trade_case_id": payload.trade_case_id, "status": "queued", "provider": "bland_ai",
        "recording_enabled": False, "created_at": _now(), "updated_at": _now(),
    })
    return {"status": "queued", "call_id": call_id, "provider": "bland_ai", "recording_enabled": False}
...
async def webhook(request: Request, x_bland_signature: str | None = Header(None, alias="X-Bland-Signature")):
    ...
    # If a secret is configured, require a matching SHA-256 body signature or exact shared token.
    if secret:
    ...
    await _store({
        ...
        "contact_name": metadata.get("contact_name"), "company": metadata.get("company"),
...
async def inbound_event(request: Request):
    """Optional normalized inbound event endpoint for Bland pathway/webhook routing."""
    payload = await request.json()
    call_id = str(payload.get("call_id") or payload.get("id") or f"call_{secrets.token_urlsafe(12)}")
    await _store({
        "call_id": call_id, "direction": "inbound", "phone_number": payload.get("from") or payload.get("phone_number"),
        "status": payload.get("status") or "received", "provider": "bland_ai", "recording_enabled": False,
        "provider_payload": payload, "created_at": _now(), "updated_at": _now(),
    })
    return {"status": "accepted", "call_id": call_id, "agent_policy": _task()}
