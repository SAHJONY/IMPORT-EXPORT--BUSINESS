...
function EmergencyScreen({message='The application encountered a client-side error.'}:{message?:string}){
  return <main style={{minHeight:'100vh',background:'#050b13',color:'#f5f9ff',fontFamily:'Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',padding:'28px'}}><div style={{maxWidth:980,margin:'0 auto'}}><div style={{fontSize:12,fontWeight:900,letterSpacing:'.16em',color:'#5ad8ff'}}>SAHJONY GLOBAL TRADE</div><h1 style={{fontSize:'clamp(42px,7vw,86px)',lineHeight:.92,letterSpacing:'-.05em',margin:'28px 0 18px'}}>Global Trade Operating System</h1><p style={{maxWidth:720,color:'#9db2c5',fontSize:17,lineHeight:1.6}}>{message}</p><div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:24}}><a href="/" style={linkPrimary}>Business site</a><a href="/start" style={linkSecondary}>Start sourcing request</a><a href="/owner" style={linkSecondary}>Owner OS</a><a href="/sign-in?role=employee" style={linkSecondary}>Employee</a><a href="/sign-in?role=customer" style={linkSecondary}>Customer</a></div></div></main>
}
...
function hardFallback(reason:string){
 ...
 rootElement.innerHTML=`<main style="min-height:100vh;background:#050b13;color:#f5f9ff;font-family:Inter,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;padding:28px"><div style="max-width:980px;margin:0 auto"><div style="font-size:12px;font-weight:900;letter-spacing:.16em;color:#5ad8ff">SAHJONY GLOBAL TRADE</div><h1>Global Trade Operating System</h1><p>${safe}</p><a href="/">Business site</a></div></main>`
}
window.addEventListener('error',e=>{console.error('SAHJONY_WINDOW_ERROR',e.error||e.message);setTimeout(()=>{if(rootElement&&!rootElement.textContent?.trim())hardFallback('A browser error interrupted the interface.')},0)});
window.addEventListener('unhandledrejection',e=>{console.error('SAHJONY_UNHANDLED_REJECTION',e.reason);setTimeout(()=>{if(rootElement&&!rootElement.textContent?.trim())hardFallback('A browser operation failed unexpectedly.')},0)});
...
function boot(){
 ...
 if(path==='/sign-in'||path.startsWith('/sign-in/')){createRoot(rootElement).render(<StrictMode><AppBoundary><NeonAuthPage/></AppBoundary></StrictMode>);return}
 ...
 try{createRoot(rootElement).render(<StrictMode><AppBoundary><App/></AppBoundary></StrictMode>)}catch(error){console.error('SAHJONY_BOOT_FAILURE',error);hardFallback('The application could not initialize.')}
}
boot();
