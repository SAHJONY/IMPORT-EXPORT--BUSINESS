...
export default function DealPriorityEngine(){
 const [rows,setRows]=useState<Ranked[]>([]);const [loading,setLoading]=useState(true);const [error,setError]=useState('');
 useEffect(()=>{void(async()=>{try{const r=await ownerDataFetch('/canonical-deals.json');const j=await r.json() as {deals?:Deal[]};const deals:Deal[]=Array.isArray(j.deals)?j.deals:[];setRows(deals.map(rank).sort((a:Ranked,b:Ranked)=>b.qualityScore-a.qualityScore))}catch(e){setError(e instanceof Error?e.message:'Unable to load owner records')}finally{setLoading(false)}})()},[]);
 ...
 return <main style={s.page}>
  <header style={s.header}><div><div style={s.kicker}>OWNER OS · CAPITAL ALLOCATION</div><h1 style={s.h1}>Quality-adjusted deal priority</h1><p style={s.lead}>Allocate AI research, sourcing and follow-up to opportunities with the strongest expected gross profit per unit of time and risk. Scores are decision aids, not booked revenue.</p><div style={s.clock}>Houston operating time · {houstonNow}</div></div><nav style={s.nav}><a style={s.link} href="/owner/dashboard">Dashboard</a><a style={s.link} href="/owner/economics">Economics</a><a style={s.link} href="/owner/exceptions">Exceptions</a></nav></header>{error&&<p role="alert" style={{color:"#ff9b9b",padding:16}}>{error}</p>}
