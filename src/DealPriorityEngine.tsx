...
type Ranked={deal:Deal;probability:number;daysToClose:number;riskFactor:number;economicValue:number;expectedGrossProfit:number;qualityScore:number;band:'EXECUTE NOW'|'ADVANCE'|'DE-RISK'|'HOLD';reason:string};
...
function rank(deal:Deal):Ranked{
 ...
 const qualityScore=econ>0?Math.round((expected/Math.max(1,daysToClose)/risk)*100)/100:0;
 const band:Ranked['band']=stage==='BLOCKED'||risk>=1.6?'DE-RISK':qualityScore>=20?'EXECUTE NOW':qualityScore>=5?'ADVANCE':'HOLD';
 const reason=band==='EXECUTE NOW'?'Highest quality-adjusted expected gross profit per day.':band==='ADVANCE'?'Positive economics and maturity justify active follow-up.':band==='DE-RISK'?'Commercial value may exist, but capital/payment/counterparty risk must be reduced first.':'Insufficient evidenced economics or low near-term expected value.';
 return {deal,probability,daysToClose,riskFactor:risk,economicValue:econ,expectedGrossProfit:expected,qualityScore,band,reason};
}
...
export default function DealPriorityEngine(){
 ...
 return <main style={s.page}>
  <header style={s.header}><div><div style={s.kicker}>OWNER OS · CAPITAL ALLOCATION</div><h1 style={s.h1}>Quality-adjusted deal priority</h1><p style={s.lead}>Allocate AI research, sourcing and follow-up to opportunities with the strongest expected gross profit per unit of time and risk. Scores are decision aids, not booked revenue.</p></div><nav style={s.nav}><a style={s.link} href="/owner/dashboard">Dashboard</a><a style={s.link} href="/owner/economics">Economics</a><a style={s.link} href="/owner/exceptions">Exceptions</a></nav></header>
  <section style={s.metrics}><article style={s.metric}><small>Execute now</small><strong>{execute}</strong></article><article style={s.metric}><small>De-risk first</small><strong>{derisk}</strong></article><article style={s.metric}><small>Probability-adjusted gross profit</small><strong>{money(totalExpected)}</strong></article><article style={s.metric}><small>Governed opportunities</small><strong>{rows.length}</strong></article></section>
  <section style={s.panel}><div style={s.kicker}>CEO WORK QUEUE</div><h2 style={s.h2}>{loading?'Calculating…':'Highest economic value per day first'}</h2><div style={s.tableWrap}><table style={s.table}><thead><tr><th>Rank</th><th>Opportunity</th><th>Decision</th><th>Economic value</th><th>Close probability</th><th>Expected GP</th><th>Days</th><th>Risk factor</th><th>QAEV score</th><th>Next action</th></tr></thead><tbody>{rows.map((r,i)=><tr key={r.deal.id}><td><b>{i+1}</b></td><td><strong>{r.deal.title}</strong><div style={s.muted}>{r.deal.market||'Market pending'} · {r.deal.stage||'LEAD'}</div></td><td><span style={{...s.pill,...tone(r.band)}}>{r.band}</span><div style={s.muted}>{r.reason}</div></td><td>{money(r.economicValue)}</td><td>{Math.round(r.probability*100)}%</td><td><strong>{money(r.expectedGrossProfit)}</strong></td><td>{r.daysToClose}</td><td>{r.riskFactor.toFixed(1)}×</td><td><strong>{r.qualityScore.toFixed(2)}</strong></td><td>{r.deal.nextAction||'Define next action'}</td></tr>)}</tbody></table></div></section>
  <section style={s.policy}><strong>Formula:</strong> quality-adjusted expected value = economic value × estimated close probability ÷ expected days to close ÷ risk factor. Only governed deal data is used. Missing economics score zero until supported inputs exist.</section>
 </main>
}
...
const s:Record<string,any>={page:{minHeight:'100vh',background:'#03070c',color:'#f4f8fb',fontFamily:'Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',padding:28},header:{maxWidth:1500,margin:'0 auto',display:'flex',justifyContent:'space-between',gap:24,alignItems:'end',padding:'28px 0 36px',borderBottom:'1px solid rgba(255,255,255,.08)'},kicker:{fontSize:10,fontWeight:950,letterSpacing:'.17em',color:'#66dcff'},h1:{fontSize:'clamp(42px,6vw,78px)',letterSpacing:'-.055em',lineHeight:.95,margin:'12px 0'},lead:{maxWidth:850,color:'#8fa4b5',lineHeight:1.6},nav:{display:'flex',gap:10,flexWrap:'wrap'},link:{color:'#d7e6ee',textDecoration:'none',padding:'10px 13px',border:'1px solid rgba(255,255,255,.1)',borderRadius:999,fontSize:12},metrics:{maxWidth:1500,margin:'16px auto',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12},metric:{padding:20,border:'1px solid rgba(255,255,255,.08)',borderRadius:16,background:'#09131d'},panel:{maxWidth:1500,margin:'16px auto',padding:20,border:'1px solid rgba(255,255,255,.08)',borderRadius:18,background:'#07111a'},h2:{margin:'8px 0 18px',fontSize:28},tableWrap:{overflow:'auto'},table:{width:'100%',minWidth:1450,borderCollapse:'collapse'},muted:{fontSize:11,color:'#7f95a6',lineHeight:1.45,marginTop:5},pill:{display:'inline-block',padding:'5px 8px',borderRadius:999,fontSize:10,fontWeight:900,marginBottom:4},good:{background:'rgba(54,211,153,.12)',color:'#69efaa'},info:{background:'rgba(102,220,255,.12)',color:'#77e3ff'},warn:{background:'rgba(255,180,84,.12)',color:'#ffbd6d'},neutral:{background:'rgba(255,255,255,.06)',color:'#b5c4ce'},policy:{maxWidth:1500,margin:'16px auto',padding:16,borderLeft:'3px solid #66dcff',background:'rgba(102,220,255,.055)',color:'#9fb3c2',fontSize:12,lineHeight:1.6}};
