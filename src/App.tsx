...
function CommandPalette({role,query,setQuery,results,close}:any){return <div className="command-overlay" onMouseDown={close}><section className="command-palette" role="dialog" aria-modal="true" aria-label="Search SAHJONY workspaces" onMouseDown={e=>e.stopPropagation()}><div className="command-search"><span aria-hidden="true">⌕</span><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search workspaces, sourcing, shipments, compliance…" aria-label="Search workspaces"/><kbd aria-hidden="true">ESC</kbd></div><div className="command-results" role="listbox" aria-label="Workspace results">{results.length?results.map((item:any)=><button role="option" key={item.key} onClick={()=>{nav(rolePath(role,item.key));close()}}><small>{item.group}</small><strong>{item.label}</strong><span>{item.description}</span></button>):<div className="empty-command" role="status">No matching workspace</div>}</div></section></div>}
...
function HiggsfieldStudio({api}:any){
 ...
 const [error,setError]=useState('');
 useEffect(()=>{api('/higgsfield-cloud/health',{cache:'no-store'}).then(setHealth).catch((e:any)=>setError(e.message))},[]);
 async function generate(scene:string,label:string){
  if(!confirm(`Generate the 5-second Seedance 2.5 clip for ${label}? This spends Higgsfield Cloud credits.`))return;
  setBusy(scene);setError('');
  try{const job=await api('/higgsfield-cloud/cinematic/generations',{method:'POST',body:JSON.stringify({scene,confirm:'GENERATE',duration:5,resolution:'720p'})});setJobs(v=>({...v,[scene]:job}))}catch(e:any){setError(e.message)}finally{setBusy('')}
 }
 async function check(scene:string){const job=jobs[scene];if(!job)return;setBusy(scene);setError('');try{const next=await api(`/higgsfield-cloud/cinematic/generations/${encodeURIComponent(job.request_id)}`,{cache:'no-store'});const result=next.result||{};const url=result.video?.url||result.videos?.[0]?.url||result.url;setJobs(v=>({...v,[scene]:{...job,status:next.status,url}}))}catch(e:any){setError(e.message)}finally{setBusy('')}}
 return <section className="workspace-section hf-studio" data-cinematic data-cinematic-reveal><div className="section-title"><div><span className="eyebrow">CINEMATIC PRODUCTION</span><h2>Higgsfield Cloud · Seedance 2.5</h2><p>Six controlled homepage scenes. Credentials stay server-side in Vercel; every paid generation requires Owner confirmation.</p></div><span className={`hf-health ${health?.credentials_configured?'ready':''}`}>{health?.credentials_configured?'CLOUD READY':health?'CONFIGURATION REQUIRED':'CHECKING'}</span></div>{error&&<div className="error-banner">{error}</div>}<div className="hf-scenes">{scenes.map(([id,n,label])=>{const job=jobs[id];return <article key={id}><small>{n} / 06</small><strong>{label}</strong><span>{job?`${job.status} · ${job.request_id.slice(0,12)}…`:'5 sec · 720p · 16:9'}</span><div>{!job?<button disabled={busy===id||!health?.credentials_configured} onClick={()=>generate(id,label)}>{busy===id?'Submitting…':'Generate clip'}</button>:job.url?<a href={job.url} target="_blank" rel="noreferrer">Open video ↗</a>:<button disabled={busy===id} onClick={()=>check(id)}>{busy===id?'Checking…':'Check status'}</button>}</div></article>})}</div></section>
}
