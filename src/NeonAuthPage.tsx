...
type AppRole='customer'|'employee'|'ai_operator';

function nextPath(role:AppRole){
  const portalRole=role==='ai_operator'?'employee':role;
  const params=new URLSearchParams(location.search);
  const requested=params.get('next')||`/${portalRole}`;
  return requested.startsWith(`/${portalRole}`)?requested:`/${portalRole}`;
}

function requestedRole():AppRole{
  const value=new URLSearchParams(location.search).get('role');
  return value==='ai_operator'?'ai_operator':value==='employee'?'employee':'customer';
}
...
export default function NeonAuthPage(){
  ...
  async function finish(accessToken?:string){
    ...
    const backendRole=role==='ai_operator'?'employee':role;
    const check=await fetch('/identity/session',{headers:{Authorization:`Bearer ${jwtToken}`,'X-Role':backendRole}});
    ...
    const portalRole=role==='ai_operator'?'employee':role;
    sessionStorage.setItem(`sahjony.${portalRole}.token`,jwtToken);
    if(role==='ai_operator'){
      sessionStorage.setItem('sahjony.employee.id',String(body.user_id||body.email||'ai-operator'));
      sessionStorage.setItem('sahjony.operator.role','ai_operator');
      sessionStorage.setItem('sahjony.operator.title','Co-CEO · AI Operator');
    }else if(role==='employee'){
      sessionStorage.setItem('sahjony.employee.id',String(body.user_id||body.email||'staff'));
    }
  ...
  async function submit(e:React.FormEvent){
    ...
    try{
      if(!supabase)throw new Error('Supabase frontend configuration is not active.');
      if(mode==='signup'){
        if(role!=='customer')throw new Error('Privileged accounts must be approved by SAHJONY. Public privileged signup is disabled.');
  ...
  return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 15% 0,rgba(90,216,255,.15),transparent 30%),#050b13',color:'#f5f9ff',fontFamily:'Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',display:'grid',placeItems:'center',padding:20}}>
    <section style={{width:'min(100%,520px)',border:'1px solid rgba(255,255,255,.13)',borderRadius:24,background:'linear-gradient(180deg,#0d1d2c,#081522)',padding:28,boxShadow:'0 35px 100px rgba(0,0,0,.35)'}}>
      ...
      <h1 style={{fontSize:42,lineHeight:.96,letterSpacing:'-.045em',margin:'12px 0 10px'}}>{role==='ai_operator'?'AI Operator · Co-CEO access':role==='employee'?'Employee access':'Customer access'}</h1>
      <p style={{color:'#9fb4c6',lineHeight:1.6,marginBottom:22}}>{role==='ai_operator'?'Dedicated AI Operator identity for governed business operations. Chairman-only authority remains separately controlled.':role==='employee'?'Sign in with an approved SAHJONY employee identity. A normal public account cannot enter employee operations.':'Create or sign in to your secure trade workspace. Authentication and application data are managed by Supabase and access is verified by SAHJONY.'}</p>
      ...
      <div style={{marginTop:22,paddingTop:16,borderTop:'1px solid rgba(255,255,255,.1)',color:'#718aa0',fontSize:11,lineHeight:1.55}}>Chairman authentication remains separate and restricted. Public signup never grants employee, compliance, finance, admin, release, or Chairman authority.</div>
      {role!=='customer'&&<a href="/sign-in?role=customer" style={{display:'inline-block',marginTop:14,color:'#9fb4c6',fontSize:11}}>Customer sign in</a>}
    </section>
  </main>
}
