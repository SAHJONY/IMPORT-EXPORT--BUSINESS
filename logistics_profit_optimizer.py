...
class RouteCandidate:
    ...
    def operating_cost(self) -> Decimal:
        return sum([
            self.linehaul_cost,
            self.international_freight,
            self.origin_handling,
            self.destination_handling,
            self.last_mile,
...
def _service_score(r: RouteCandidate) -> int:
    score = 0
    score += 18 if r.tracking else 0
    score += 18 if r.pod else 0
    score += 14 if r.claims else 0
    score += 10 if r.pickup else 0
    score += 12 if r.door_delivery else 0
    score += 16 if r.capacity_confirmed else 0
    score += 12 if r.compliance_cleared else 0
...
def optimize_route(
    ...
    *,
    ...
    target_margin = policy.target_margin_pct
    if route.service_level in {'EXPRESS','PREMIUM'}:
        target_margin = policy.premium_margin_pct
    if customer_type == 'AGENCY':
        target_margin = max(policy.minimum_margin_pct, target_margin - Decimal('4.00'))

    floor_price = max(
        _price_for_margin(fully_loaded, policy.minimum_margin_pct),
        fully_loaded + policy.minimum_gp_usd,
    )
    target_price = _price_for_margin(fully_loaded, target_margin)

    competitor = money(competitor_effective_price) if competitor_effective_price is not None else None
    ...
    if competitive_target is not None and competitive_target >= floor_price:
    ...
    booking_ready = bool(route.capacity_confirmed and route.compliance_cleared)
    quote_status = 'FIRM_QUOTE_ELIGIBLE' if booking_ready else 'PRELIMINARY_COMPLIANCE_OR_CAPACITY_PENDING'
    ...
    return {
        ...
        'customer_safe': {
            ...
            'pod': route.pod,
            'claims_support': route.claims,
            'pickup': route.pickup,
            'door_delivery': route.door_delivery,
        },
    }
