...
app = FastAPI(title="SAHJONY Bland Outbound Transport", version="1.2.0", docs_url=None, redoc_url=None)
...
def _normalize_language(value: str | None) -> str | None:
    ...
    aliases = {"english": "en-US", "en": "en-US", "en-us": "en-US", "spanish": "es", "español": "es", "es": "es", "french": "fr", "français": "fr", "fr": "fr", "portuguese": "pt", "português": "pt", "pt": "pt", "german": "de", "deutsch": "de", "de": "de", "italian": "it", "italiano": "it", "it": "it", "arabic": "ar", "العربية": "ar", "ar": "ar", "babel": "babel", "multilingual": "babel"}
...
def _task(context: str | None, language: str | None) -> str:
    ...
    return (
        "You are the outbound business phone agent for SAHJONY Global Trade. Identify SAHJONY Global Trade naturally, detect the person's language and respond in that language. "
        f"Initial language hint: {language_hint}. "
        "Determine whether the contact is a useful buyer, supplier, partner, service provider, institutional counterparty or follow-up opportunity and capture only factual information needed for CRM and the next commercial action. "
        "When relevant, confirm full name, company, role, business email, country, business requirement or capability, specifications, quantity or capacity, destination, timing, Incoterm preference, payment or document readiness, decision authority and next step. Ask only what matters. "
        "For buyers, focus on the requirement, specification, quantity, destination, timing, Incoterm, documentation, payment capability and decision authority. "
        "For suppliers, focus on capability, specifications, capacity, MOQ, certifications or documents, indicative commercial terms if offered, loading point, lead time, payment terms and availability. "
        "The post-call system may autonomously create or update a CRM prospect only from facts actually provided. Never invent an email, identity, company, country, requirement, quantity, destination or qualification facts. "
        "Mark a commercial lead qualified only when the conversation provides meaningful evidence of fit, need or capability and a viable next step. This is not KYC approval, credit approval, sanctions clearance, supplier approval, contracting or transaction approval. "
        "Preserve exact names, companies, emails, specifications, quantities, currencies, Incoterms, locations, legal terms and numbers. Confirm spelling when uncertain. "
        "Never bind SAHJONY to price, purchase, sale, financing, payment, exclusivity, contract, KYC approval or sanctions approval. Respect do-not-call requests immediately. Do not intentionally record the call. End with a concise confirmed next step. "
...
async def _autonomous_crm(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (email and contact_name and company):
        await _store("customer_crm_audit", {"event_id": f"voice_audit_{secrets.token_urlsafe(12)}", "customer_id": None, "intake_id": None, "actor": "voice_autonomous_agent", "event_type": "voice_lead_deferred_missing_identity", "payload": {"call_id": payload.get("call_id"), "disposition": disposition, "has_email": bool(email), "has_contact_name": bool(contact_name), "has_company": bool(company), "summary": summary}, "created_at": now})
    ...
    customer_row = {**(existing or {}), "customer_id": customer_id, "legal_name": company, "trade_name": (existing or {}).get("trade_name"), "contact_name": contact_name, "email": email, "phone": phone or (existing or {}).get("phone"), "country_code": metadata.get("country_code") or (existing or {}).get("country_code"), "status": "PROSPECT", "sales_status": sales_status, "source": "VOICE_AUTONOMOUS", "last_voice_call_id": payload.get("call_id"), "last_voice_disposition": disposition or None, "last_voice_summary": summary or None, "created_at": (existing or {}).get("created_at") or now, "updated_at": now}
    await backend.insert("customer_accounts", customer_row)
    requirement = str(metadata.get("product_need") or "").strip()
    destination = str(metadata.get("destination_country") or "").strip()
    intake_id = None
    if requirement and destination:
        intake_id = f"intake_voice_{secrets.token_urlsafe(10)}"
        await backend.insert("customer_trade_intakes", {"intake_id": intake_id, "customer_id": customer_id, "product_need": requirement, "destination_country": destination, "notes": summary or transcript[:3000], "status": "NEW", "qualification_status": qualification_status, "source": "VOICE_AUTONOMOUS", "source_call_id": payload.get("call_id"), "created_at": now, "updated_at": now})
    await backend.insert("customer_crm_audit", {"event_id": f"voice_audit_{secrets.token_urlsafe(12)}", "customer_id": customer_id, "intake_id": intake_id, "actor": "voice_autonomous_agent", "event_type": "voice_autonomous_qualification", "payload": {"call_id": payload.get("call_id"), "disposition": disposition, "sales_status": sales_status, "qualification_status": qualification_status, "summary": summary}, "created_at": now})
...
async def outbound(payload: OutboundCall, authorization: str | None = Header(None, alias="Authorization")):
    ...
    metadata = {"lead_id": payload.lead_id, "trade_case_id": payload.trade_case_id, "contact_name": payload.contact_name, "company": payload.company, "email": payload.email, "country_code": payload.country_code, "product_need": payload.product_need, "destination_country": payload.destination_country, "phone_number": phone_number, "language_hint": language or "auto", "autonomous_crm": payload.autonomous_crm, "source": "sahjony_global_trade_os"}
    body: dict[str, Any] = {"phone_number": phone_number, "from": from_number, "task": _task(payload.context, language), "record": False, "metadata": metadata, "summary_prompt": "Summarize only factual commercial information stated during the call: identity/company, business role, requirement or capability, specifications, quantity/capacity, destination, timing, commercial terms discussed, missing information, objections and agreed next step. Do not infer approvals or facts that were not stated.", "dispositions": ["QUALIFIED_LEAD", "FOLLOW_UP_REQUIRED", "NEEDS_MORE_INFO", "NOT_INTERESTED", "NOT_QUALIFIED", "DO_NOT_CONTACT", "NO_CONTACT_MADE"], "webhook": f"{_base_url().rstrip('/')}/voice/webhook"}
    if language:
