...
app = FastAPI(title='SAHJONY Cuba Private Sector Fuels Desk', version='1.0.0', docs_url=None, redoc_url=None)
...
AUTHORITY_NOTES = {
    'bis': 'BIS Cuba guidance states certain U.S.-origin gas and petroleum exports to eligible Cuban private-sector entities and individual consumers may use License Exception SCP when all conditions in 15 CFR 740.21 are met.',
    ...
    'banking_2026': 'BIS guidance updated March 4, 2026 suspends SCP for transactions involving deposit of foreign funds into a Cuban-owned bank; this suspension does not apply to qualifying transactions using third-country banks or other payment systems that do not involve such deposits.',
}
...
async def create_case(payload: FuelCaseIn, authorization: str | None = Header(None, alias='Authorization')):
    owner(authorization)
    ts = now(); fid = f'cuf_{secrets.token_urlsafe(12)}'
    risk_flags = []
    ...
    await get_backend().insert('cuba_private_fuel_cases', row)
...
async def list_cases(authorization: str | None = Header(None, alias='Authorization')):
    ...
    return {'cases':rows,'authority':'PRIVATE_SECTOR_FUEL_TRADE_PREPARATION_ONLY'}
...
async def detail(case_id: str, authorization: str | None = Header(None, alias='Authorization')):
    ...
    return {'case':row,'gates':gates,'gate_state':await gate_state(case_id),'authority_notes':AUTHORITY_NOTES}
...
async def approve(case_id: str, authorization: str | None = Header(None, alias='Authorization')):
    ...
    if incomplete:
        raise HTTPException(409, {'message':'Cuba private-sector fuel gates incomplete','gates':incomplete})
