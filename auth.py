...
def _membership(user_id: str, required_roles: set[str] | None = None) -> dict | None:
    ...
    try:
        params = {
            "select": "role,status,customer_id,employee_id,mfa_required",
...
def verify_owner_totp(provided: str, *, at_time: int | None = None, window: int = 1) -> bool:
    if not provided or len(provided) != 6 or not provided.isdigit() or not owner_totp_configured():
        return False
...
def _supabase_aal2(claims: dict) -> bool:
    return str(claims.get("aal") or "").lower() == "aal2"
...
def decode_owner_session(token: str) -> dict | None:
    ...
    if claims:
        ...
        if membership and (not owner_mfa_required() or _supabase_aal2(claims)):
            return {
                ...
                "mfa_verified": (not owner_mfa_required()) or _supabase_aal2(claims),
                "identity_provider": "supabase_auth",
            }
    ...
    try:
        ...
        if not hmac.compare_digest(_b64url_decode(supplied_signature), expected):
            return None
...
def verify_customer_token(provided: str):
    ...
    return {
        ...
        "business_id": membership.get("customer_id"),
        "email": claims.get("email"),
        "name": metadata.get("name") or metadata.get("full_name"),
        "identity_provider": "supabase_auth",
        "app_role": membership.get("role"),
    }
...
def verify_participant(token: str | None = Header(None, alias="Authorization")):
    ...
    if not info:
        raise HTTPException(403, "Invalid or expired Supabase identity or inactive membership")
    return info
