(()=>{
  ...
  const ORIGINAL='sahjony.originalText';
  const defaultLocale=document.documentElement.lang||'en-US';
  let active=localStorage.getItem(STORAGE)||defaultLocale;
  let busy=false;
  ...
  async function translate(locale){
    if(busy)return;active=locale;localStorage.setItem(STORAGE,locale);
    if(locale===defaultLocale){restore();setState('ok','Original');return;}
    busy=true;setState('busy','Translating…');
    try{
      ...
      for(const chunk of chunks){
        const texts=chunk.map(n=>(n.nodeValue||'').trim());const key=locale+'|'+texts.join('\u241e');let result=cache.get(key);
        if(!result){const r=await fetch('/language/ui-translate-batch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({texts,target_locale:locale,source_type:'ui'})});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.detail||'Translation unavailable');result=j;cache.set(key,result);}
      ...
      document.documentElement.lang=locale;setState('ok',locale);
  ...
  async function mount(){
    ...
    document.body.appendChild(wrap);const select=wrap.querySelector('select');const list=await locales();
    ...
    select.addEventListener('change',()=>translate(select.value));wrap.querySelector('button').addEventListener('click',()=>{select.value=defaultLocale;translate(defaultLocale)});
    if(active!==defaultLocale)translate(active);else setState('ok','Original');
  }
