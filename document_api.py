...
app = FastAPI(title="SAHJONY Global Trade Documents", version="1.2.0", docs_url=None, redoc_url=None)
...
class DocumentCreate(BaseModel):
    ...
    supersedes_document_id: str | None = Field(default=None,max_length=180)
...
def identity(role, authorization, employee_id):
    ...
    if not c: raise HTTPException(403,"Verified customer identity required")
...
async def health():
    return {"status":"ok","service":"document-movement","version":"1.2.0","configured":bool(os.getenv("INSFORGE_BASE_URL") and os.getenv("INSFORGE_API_KEY")),"storage_enabled":os.getenv("INSFORGE_STORAGE_ENABLED","false").lower()=="true","server_derived_storage_keys":True,"timeline_integration":True}
...
async def create_document(payload: DocumentCreate, x_role: str|None=Header(None,alias="X-Role"), authorization: str|None=Header(None,alias="Authorization"), x_employee_id: str|None=Header(None,alias="X-Employee-Id")):
    ...
    if payload.supersedes_document_id:
        prior=await get_backend().select("trade_documents",params={"document_id":f"eq.{payload.supersedes_document_id}","limit":"1"}) or []
        if not prior: raise HTTPException(404,"Superseded document not found")
        if prior[0].get("trade_case_id")!=payload.trade_case_id: raise HTTPException(409,"Superseded document must belong to the same trade case")
        if actor["role"]=="customer" and prior[0].get("customer_id")!=actor["id"]: raise HTTPException(403,"Customer scope mismatch")
    status="customer_submitted" if actor["role"]=="customer" else "requested"
    doc_id=f"doc_{secrets.token_urlsafe(16)}"; ts=now()
    row={"document_id":doc_id,"trade_case_id":payload.trade_case_id,"customer_id":customer_id,"document_type":payload.document_type,"title":payload.title,
         "original_filename":None,"storage_object_key":None,"content_type":None,"size_bytes":None,"object_etag":None,"checksum_sha256":None,
         "version":payload.version,"supersedes_document_id":payload.supersedes_document_id,"storage_status":"metadata_only","malware_scan_status":"not_started",
         "status":status,"customer_visible":payload.customer_visible if actor["role"]!="customer" else True,"legal_hold":False,
         "created_by_role":actor["role"],"created_by_id":actor["id"],"created_at":ts,"updated_at":ts}
    try:
        result=await get_backend().insert("trade_documents",row)
        movement=await audit(doc_id,actor,None,status,"Document registered; storage key will be generated by secure storage service")
    ...
    return {"document":row,"movement":movement,"timeline_event":timeline,"persistence":result,"next":"Use /document-storage/{document_id}/upload to obtain a short-lived signed upload URL"}
...
async def list_documents(trade_case_id: str|None=Query(default=None,max_length=160), customer_id: str|None=Query(default=None,max_length=160), x_role: str|None=Header(None,alias="X-Role"), authorization: str|None=Header(None,alias="Authorization"), x_employee_id: str|None=Header(None,alias="X-Employee-Id")):
    ...
    for row in rows or []:
        ...
        if actor["role"]=="customer":
            item.pop("storage_object_key",None); item.pop("malware_scan_reference",None)
        safe.append(item)
    return {"documents":safe,"actor":actor}
...
async def move_document(document_id: str, payload: MoveRequest, x_role: str|None=Header(None,alias="X-Role"), authorization: str|None=Header(None,alias="Authorization"), x_employee_id: str|None=Header(None,alias="X-Employee-Id")):
    ...
    if payload.to_status in {"owner_review","approved","released"}:
        if doc.get("storage_status")!="clean" or doc.get("malware_scan_status") not in {"clean","waived"}:
            raise HTTPException(423,"Document cannot enter owner review/approval/release until secure storage verification and malware gate are clear")
...
async def movements(document_id: str, x_role: str|None=Header(None,alias="X-Role"), authorization: str|None=Header(None,alias="Authorization"), x_employee_id: str|None=Header(None,alias="X-Employee-Id")):
    actor=identity(x_role,authorization,x_employee_id)
    docs=await get_backend().select("trade_documents",params={"document_id":f"eq.{document_id}","limit":"1"}) or []
