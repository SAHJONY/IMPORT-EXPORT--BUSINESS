...
def evaluate_production_readiness(*, runtime_ok: bool = True, e2e_ok: bool | None = None, connector_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    gates=[
        ...
        ReadinessGate('all_schemas_applied',_true('INSFORGE_SCHEMAS_APPLIED'),True,'All production schemas applied and checked','Apply every current InsForge schema, including country_activation, Cuba current/transition, and Cuba authorized trade.'),
        ...
        ReadinessGate('cuba_authorized_trade_desk',_true('CUBA_AUTHORIZED_TRADE_E2E_VERIFIED'),True,'Cuba employee authorization, verified authority, transaction gates, HOLD, owner release and audit E2E verified','Apply cuba_authorized_trade.sql and run a production-safe US->CU authorized-trade test proving employees cannot self-release and unverified/out-of-scope cases remain HOLD.'),
