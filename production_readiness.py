...
def evaluate_production_readiness(*, runtime_ok: bool = True, e2e_ok: bool | None = None, connector_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    gates=[
        ...
        ReadinessGate('global_supplier_sourcing',_true('GLOBAL_SUPPLIER_SOURCING_E2E_VERIFIED'),True,'Worldwide candidate sourcing, origin/destination corridor controls and owner supplier selection verified','Apply global_supplier_sourcing.sql; run a permitted corridor E2E and prove BLOCKED/LIMITED candidates cannot be selected.'),
        ReadinessGate('managed_trade_gateway',_true('MANAGED_TRADE_GATEWAY_E2E_VERIFIED'),True,'SAHJONY request intake, supplier sourcing/due diligence, role assignment, milestone gating and owner release verified','Apply managed_trade_gateway.sql and run an E2E managed transaction.'),
        ReadinessGate('intermediary_mode',_true('INTERMEDIARY_MODE_E2E_VERIFIED'),True,'Broker/agent engagement, disclosed economics, legal-role assignments, title/funds controls and database release guard verified','Apply managed_trade_intermediary.sql and run intermediary E2E tests.'),
        ReadinessGate('business_operational_controls',_true('BUSINESS_OPERATIONAL_READINESS_E2E_VERIFIED'),True,'Partners, agreements, counterparty due diligence, product dossiers, incident handling and reconciliation controls verified','Apply business_operational_readiness.sql; verify signed operating partners, approved agreement templates, KYB/sanctions dossiers, product classification dossiers and incident/claims workflow.'),
        ReadinessGate('cuba_private_business_eligibility',_true('CUBA_PRIVATE_BUSINESS_E2E_VERIFIED'),True,'Cuban private-business ownership, employee-count, government-control, screening, banking and eligibility workflow verified','Apply cuba_private_business.sql and prove ineligible or disallowed businesses cannot pass.'),
