...
def evaluate_production_readiness(*, runtime_ok: bool = True, e2e_ok: bool | None = None, connector_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    gates = [
        ...
        ReadinessGate("all_schemas_applied", _true("INSFORGE_SCHEMAS_APPLIED"), True, "All production schemas applied and checked", "Apply and verify every current InsForge schema, including country_activation.sql."),
        ReadinessGate("global_country_activation", _true("COUNTRY_ACTIVATION_E2E_VERIFIED"), True, "Country/corridor activation controls and owner approval flow verified", "Apply country_activation.sql and verify BLOCKED-by-default countries, control recalculation, owner approval, corridor gates and product-country permissions."),
