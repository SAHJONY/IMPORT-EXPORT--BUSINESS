...
def evaluate_production_readiness(*, runtime_ok: bool = True, e2e_ok: bool | None = None, connector_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    gates=[
        ...
        ReadinessGate('all_schemas_applied',_true('INSFORGE_SCHEMAS_APPLIED'),True,'All production schemas applied and checked','Apply every current InsForge schema, including managed_trade_intermediary.sql.'),
        ...
        ReadinessGate('intermediary_mode',_true('INTERMEDIARY_MODE_E2E_VERIFIED'),True,'Broker/agent engagement, disclosed economics, legal-role assignments, title/funds controls and database release guard verified','Apply managed_trade_intermediary.sql and run E2E tests proving broker/agent cases cannot take title, undisclosed compensation cannot activate, required legal roles must be verified, and release is blocked without active engagement/economics.'),
