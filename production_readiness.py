...
def evaluate_production_readiness(*, runtime_ok: bool = True, e2e_ok: bool | None = None) -> dict[str, Any]:
    ...
    gates = [
        ...
        ReadinessGate("owner_mfa", os.getenv("OWNER_MFA_REQUIRED", "false").lower() == "true", True, "Owner MFA policy enabled", "Require MFA for owner/admin access."),
        ReadinessGate("restricted_party_screening", _present("TRADE_GOV_API_KEY") or os.getenv("OFAC_DIRECT_SCREENING", "false").lower() == "true", True, "Government screening connector configured", "Configure Trade.gov CSL API or verified OFAC direct screening."),
        ...
        ReadinessGate("durable_documents", os.getenv("INSFORGE_STORAGE_ENABLED", "false").lower() == "true", True, "InsForge Storage enabled", "Enable production document storage and retention policies."),
        ReadinessGate("audit_retention", _present("AUDIT_RETENTION_DAYS") and int(os.getenv("AUDIT_RETENTION_DAYS", "0") or 0) >= 365, True, "Audit retention >=365 days", "Set auditable retention to at least 365 days."),
        ReadinessGate("backup_policy", os.getenv("BACKUPS_ENABLED", "false").lower() == "true", True, "Backups enabled", "Enable scheduled database/storage backups and restore testing."),
        ReadinessGate("monitoring_alerts", _present("ALERT_WEBHOOK_URL") or os.getenv("VERCEL_MONITORING_ENABLED", "false").lower() == "true", True, "Production alert path configured", "Configure runtime alerting and incident notifications."),
