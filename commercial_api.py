...
app = FastAPI(title='SAHJONY Global Trade Commercial Operations', version='1.0.0', docs_url=None, redoc_url=None)
...
async def publish(trade_case_id,actor,title,body,severity='info',action_required=False,customer_id=None,customer_visible=False):
    try:
        await get_backend().insert('business_events',{
            'event_id':f'evt_{secrets.token_urlsafe(14)}','trade_case_id':trade_case_id,'customer_id':customer_id,
            'event_type':'commercial','source':'commercial_api','source_ref':trade_case_id,'title':title,'body':body,
            'severity':severity,'action_required':action_required,'customer_visible':customer_visible,
...
async def health():
    return {'status':'ok','service':'commercial-execution','persistence':'insforge','first_live_trade_gate':True}
...
async def create_supplier(payload: MasterCreate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    ...
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot create suppliers')
    sid=f'sup_{secrets.token_urlsafe(12)}'; row={'supplier_id':sid,**payload.model_dump(),'compliance_status':'pending','quality_status':'pending','bank_verified':False,'created_at':now(),'updated_at':now()}
...
async def list_suppliers(x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Supplier master is internal')
...
async def create_buyer(payload: MasterCreate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot create buyer master records')
    bid=f'buy_{secrets.token_urlsafe(12)}'; row={'buyer_id':bid,**payload.model_dump(),'credit_status':'pending','compliance_status':'pending','created_at':now(),'updated_at':now()}
...
async def create_product(payload: ProductCreate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot create products')
    pid=f'prd_{secrets.token_urlsafe(12)}'; row={'product_id':pid,**payload.model_dump(),'active':True,'created_at':now(),'updated_at':now()}
...
async def create_corridor(payload: CorridorCreate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot create corridors')
    cid=f'cor_{secrets.token_urlsafe(12)}'; row={'corridor_id':cid,**payload.model_dump(),'active':True,'created_at':now(),'updated_at':now()}
...
async def create_quote(payload: QuoteCreate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot issue commercial quotes')
    ...
    qid=f'quo_{secrets.token_urlsafe(12)}'; row={'quote_id':qid,**payload.model_dump(exclude={'supplier_cost'}),'estimated_landed_cost':landed,'estimated_margin':margin,'estimated_margin_pct':margin_pct,'status':'draft','owner_approved':False,'created_by_role':actor['role'],'created_by_id':actor['id'],'created_at':now(),'updated_at':now()}
...
async def case_summary(trade_case_id: str, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    ...
    out={}
    for table in tables:
        rows=await get_backend().select(table,params={'trade_case_id':f'eq.{trade_case_id}','order':'created_at.desc','limit':'250'})
        out[table]=rows or []
    if actor['role']=='customer':
        out={k:v for k,v in out.items() if k in {'sales_orders','quality_inspections','trade_readiness_checks'}}
...
async def initialize_readiness(trade_case_id: str, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot initialize readiness')
...
async def update_readiness(trade_case_id: str, payload: ReadinessUpdate, x_role: str|None=Header(None,alias='X-Role'), authorization: str|None=Header(None,alias='Authorization'), x_employee_id: str|None=Header(None,alias='X-Employee-Id')):
    actor=identity(x_role,authorization,x_employee_id)
    if actor['role']=='customer': raise HTTPException(403,'Customers cannot change readiness controls')
