...
app = FastAPI(title="SAHJONY Global Trade Unified API", version="6.2.1", docs_url=None, redoc_url=None)
...
async def crm_runtime_health():
    status = persistent_backend_status()
    return {"status":"ok" if status["configured"] else "configuration_required","service":"customer-crm","public_intake":True,"fail_closed_promotion":True,"country_segmentation":True,"cuba_department_permanent":True,"global_lead_search_control_plane":True,"energy_vertical_connected":True,"external_trade_prospects":True,"external_trade_prospects_research_only":True,"persistence":status["provider"],"backend_configured":status["configured"],"operational":status["configured"],"database_url_configured":status["database_url_configured"],"insforge_configured":status["insforge_configured"]}
