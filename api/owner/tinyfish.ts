...
export default async function handler(req:any,res:any){
  if(req.method==='GET'){
    ...
    return send(res,200,{
      service:'tinyfish-governed-gateway',
      status:tinyfish&&owner?'ready':'configuration_required',
      ready:tinyfish&&owner,
      tinyfish_ready:tinyfish,
      owner_gate_ready:owner,
      fail_closed:true,
      direct_client_secret_exposure:false,
      endpoint_mode:'async',
    });
  }
  ...
  const payload:any={
    url,
    goal,
    browser_profile:body.browser_profile==='stealth'?'stealth':'lite',
    agent_config:{mode:'strict',max_steps:maxSteps},
    capture_config:{elements:true,snapshots:true,screenshots:true,recording:false,html:false},
  };
  ...
  try{
    ...
    if(!response.ok)return send(res,502,{error:'tinyfish_upstream_error',status:response.status,detail:data,fail_closed:true});
    return send(res,202,{state:'in_progress',provider:'tinyfish',run_id:data.run_id||data.id||null,budget_cents:budgetCents,max_steps:maxSteps,approval_state:approval,risk_level:risk,evidence_capture:true,upstream:data});
  }catch(error){
    return send(res,502,{error:'tinyfish_unreachable',detail:error instanceof Error?error.message:'unknown_error',fail_closed:true});
  }
}
